...
window.addEventListener('mousedown', function(e) {
  if( e.target.dataset.cursor == 'stretch' ||
    ...
    TweenLite.to(pointer, 0.3, { ease: Power4.easeOut, rotation: 0, width: pointer_size+15, height: pointer_size-10 });
  }
});

window.addEventListener('mouseup', function(e) {
  TweenLite.to(pointer, 0.3, { ease: Power4.easeOut, rotation: 45, width: pointer_size, height: pointer_size });
});
...
hammertime.on('rotate', function(ev) {
  ...
  //TweenLite.to(panner, 0.1, { rotation: ev.rotation })
});
